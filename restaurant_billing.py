...
def TotalofUnit():
    ...
    DrinksPrices = str('%.2f' % Prices_Drinks)
    FoodsPrices = str('%.2f' % Prices_Food)
    Total_of_Food.set(FoodsPrices)
    Total_of_Drinks.set(DrinksPrices)
    SC = str('%.2f' % 1.59)
    ServiceCharge.set(SC)

    Sub_Total_of_Unit = str('%.2f'%(Prices_Drinks + Prices_Food))
    SubTotal.set(Sub_Total_of_Unit)

    Tax = str('%.2f'%((Prices_Drinks + Prices_Food) * 0.15))
    PaidTax.set(Tax)

    TT = ((Prices_Drinks + Prices_Food ) * 0.15)
    TC = str('%.2f'%(Prices_Drinks + Prices_Food + TT))
    TotalCost.set(TC)
